...
import { getProductSeller } from "../redux/actions/productSeller"
import empty from "../public/images/empty-input-image.png"
import NumberFormat from "react-number-format"
import Modal from "../components/ModalDelete"
import React from "react"
import { deleteProduct } from "../redux/actions/productList"
import { useRouter } from "next/router"
import { getProfile } from "../redux/actions/auth"

const MyProduct = () => {
  // window.scrollTo(0, 0)
  const {auth, productSeller, pages} = useSelector(state=>state)
  ...
  return (
    <Layout>
        <Head>
        <title>The King | Profile Seller</title>
        ...
        {productSeller.data?.map((data, index) => {
          return (
            <Row key={data?.id} className='my-5'>
              <Col xs={12} sm={6} lg={4} className='d-flex flex-row align-items-center'>
                <Image src={data.product_images[0]?.image ? data.product_images[0]?.image : empty} alt='product' width={100} height={100} />
              ...
              <Col xs={12} sm={12} lg={4} className='my-auto'>
                <span className="fw-bold ms-0 ms-lg-5"><NumberFormat value={data?.price} displayType={'text'} thousandSeparator={'.'} decimalSeparator={','} prefix={'Rp. '} ></NumberFormat></span>
                {/* <Button onClick={deleteHandler(data.id)} className='px-5 py-2 ms-5' color='danger'>Delete</Button>
